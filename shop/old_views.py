...
from dash.models import Inventory, Category, PaymentMethod, Delivery, TimedDeal, Profile
...
def index(request, name):
    ...
    featured_products = Inventory.objects.filter(shop=shop, is_featured=True)
    cart_products = Cart.objects.filter(shop=shop, customer=request.user, status='pending').values_list('cart_product_id', flat=True)
    featured_products = featured_products.exclude(product_id__in=cart_products)[:8]
    deal = TimedDeal.objects.filter(shop=shop)[:1]
    # context = {
        'profile': profile,
    ...
    logger.debug(f'featured cats: {featured_categories}')
